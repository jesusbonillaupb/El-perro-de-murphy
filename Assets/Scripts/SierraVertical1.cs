...
public class SierraVertical1 : MonoBehaviour
{
    ...
    public void StartMoving(float distancia, float distanciax, float velocidad, bool mata)
    {
        moveDistance = distancia; // Asigna la distancia
        speed = velocidad;        // Asigna la velocidad
        muerte = mata;
        moveDistancex = distanciax;

        // Calcula la posición objetivo (hacia la izquierda)
        targetPosition = startPosition - new Vector3(moveDistancex, moveDistance, 0);
